...
turns relays on til limit switch gets pushed*/
  if (newState == 1) {//open windows
    ...
    if ((PINC & 0x10) == 1) {//negative logic switch goes to low when pushed   digitalRead(limitSw)
      //turn on the relays
      PORTB |= 0x01;
      PORTB |= 0x02;
    ...
    if ((PINC & 0x08) == 1) {//negative logic switch goes to low when pushed   digitalRead(limitSw)
      //turn on the relays
      PORTD |= 0x40;
      PORTD |= 0x80;
      //wait til switch gets pushed
